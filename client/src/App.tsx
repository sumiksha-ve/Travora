...
function TravelDesk() {
  ...
  useEffect(() => { let active = true; setBookingsLoading(true); bookingsApi.list().then((data) => { if (active) { const ids = Array.isArray(data) ? data.map((booking: any) => String(booking?.travelRequest?.id ?? "")).filter(Boolean) : []; setBookedRequestIds(new Set(ids)); } }).catch((requestError) => { if (active) setBookingError(requestError instanceof Error ? requestError.message : "Booking data is unavailable."); }).finally(() => { if (active) setBookingsLoading(false); }); return () => { active = false; }; }, [refreshKey]);
  const rows = journeys.filter((journey) => journey.status === "Approved" || journey.status === "Booked").map((journey) => bookedRequestIds.has(journey.id) ? { ...journey, status: "Booked" as const } : journey);
  ...
  async function createBooking(payload: Record<string, unknown>) {
    ...
    setBookingError("");
    toast.success("Booking created. The Travel Desk queue has been refreshed.");
    setRefreshKey((value) => value + 1);
  }
  return <div className="page-stack animate-page"><PageTitle meta={pageMeta["/travel-desk"]} /><section className="ops-hero"><div><span className="card-kicker">TODAY IN OPERATIONS</span><h2>Keep the journey moving.</h2><p>Approved journeys returned by the backend are ready for booking action.</p><Link href="/travel-desk/bookings" className="button button-primary">Open booking queue <ArrowUpRight size={16} /></Link></div><div className="ops-hero-metric"><span>Open work</span><strong>{String(approvedCount).padStart(2, "0")}</strong><small>approved trips</small></div></section><div className="stats-grid desk-stats"><StatCard label="Approved to book" value={String(approvedCount).padStart(2, "0")} detail="Live queue" icon={<Clock3 size={18} />} accent="stat-accent" /><StatCard label="Booked" value={String(bookedCount).padStart(2, "0")} detail="Live backend data" icon={<TicketCheck size={18} />} /><StatCard label="Cancellations" value={String(journeys.filter((row) => row.status === "Cancelled").length).padStart(2, "0")} detail="Current status" icon={<XCircle size={18} />} /></div><div className="panel table-panel"><div className="section-title-row"><div><span className="card-kicker">OPERATIONS QUEUE</span><h2 className="section-heading">Approved travel requests</h2></div><span className="data-note"><ShieldCheck size={13} /> Live backend data</span></div>{error && <div className="form-error-message"><XCircle size={15} />{error}</div>}{bookingError && <div className="form-error-message"><XCircle size={15} />{bookingError}</div>}{loading || bookingsLoading ? <EmptyState icon={<Clock3 size={20} />} title="Loading queue" description="Fetching approved requests and booking records from Travora." /> : rows.length ? <div className="responsive-table"><table><thead><tr><th>Journey</th><th>Route</th><th>Travel dates</th><th>Status</th><th>Action</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><div className="table-journey"><span className="avatar avatar-tiny">{row.destination.slice(0, 2).toUpperCase()}</span><span><strong>{row.destination}</strong><small>{row.id} · {row.project}</small></span></div></td><td>{row.from} → {row.to}</td><td>{row.dates}</td><td><StatusBadge status={row.status === "Approved" ? "Booking pending" : row.status} /></td><td>{row.status === "Approved" ? <button className="table-action" onClick={() => { setBookingError(""); setBookingJourney(row); }}>Add booking <ArrowUpRight size={14} /></button> : <span className="table-action table-action-muted">Booked <Check size={14} /></span>}</td></tr>)}</tbody></table></div> : <EmptyState icon={<TicketCheck size={20} />} title="No approved journeys" description="Approved travel requests will appear here for ticketing." />}</div>{bookingJourney && <BookingDialog journey={bookingJourney} onClose={() => setBookingJourney(null)} onSubmit={createBooking} />}</div>;
}
