...
function NotificationBell() {
  ...
  async function markRead(id: string | number) { setActionId(id); setError(""); try { await notificationsApi.markRead(id); await refresh(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Notification could not be marked as read."); } finally { setActionId(null); } }
  async function markAllRead() { setActionId("all"); setError(""); try { await notificationsApi.markAllRead(); await refresh(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Notifications could not be marked as read."); } finally { setActionId(null); } }
  return <div className="notification-wrap"><button type="button" className="icon-button notification-button" aria-label={`Notifications${unread ? `, ${unread} unread` : ""}`} aria-expanded={open} onClick={() => setOpen((value) => !value)}><Bell size={18} />{unread > 0 && <span className="notification-count">{unread > 9 ? "9+" : unread}</span>}</button>{open && <><button type="button" className="notification-dismiss" aria-label="Close notifications" onClick={() => setOpen(false)} /><section className="notification-panel" aria-label="Notifications"><div className="notification-panel-header"><div><span className="card-kicker">TRAVORA INBOX</span><h2>Notifications</h2></div>{unread > 0 && <button type="button" className="text-link" disabled={actionId === "all"} onClick={markAllRead}>{actionId === "all" ? "Updating…" : "Mark all as read"}</button>}</div>{loading ? <div className="notification-state"><Clock3 size={18} /> Loading notifications…</div> : error ? <div className="notification-state notification-error"><XCircle size={18} />{error}</div> : items.length ? <div className="notification-list">{items.map((item) => <div className={`notification-item ${item.read ? "read" : "unread"}`} key={item.id}><span className={`notification-type notification-type-${String(item.type || "").toLowerCase()}`}><Bell size={14} /></span><span className="notification-copy"><strong>{item.title}</strong><span>{item.message}</span><small>{formatNotificationTime(item.createdAt)}</small></span>{!item.read && <button type="button" className="notification-mark-read" disabled={actionId === item.id} onClick={() => markRead(item.id)}>{actionId === item.id ? "Updating…" : "Mark as read"}</button>}{!item.read && <span className="notification-unread-dot" />}</div>)}</div> : <div className="notification-state"><Bell size={20} /><strong>You’re all caught up</strong><span>Important travel activity will appear here.</span></div>}</section></>}</div>;
}
